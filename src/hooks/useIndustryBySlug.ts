...
export function useIndustryBySlug(slug: string) {
  ...
  // Find the industry with matching slug
  const industryFromSlug = industriesData?.data?.find((industry: Industry) => industry.slug === slug);
  ...
  return {
    industry: industryData?.data || null,
